...
/*   Updated: 2018/02/15 15:27:26 by eparisot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void		ft_parse_conv_x(va_list ap, int *f_tab, long long *tmp)
{
	if (f_tab[7])
...
static void		ft_parse_4_x(int *p_ret, int *f_tab)
{
...
static void		ft_parse_1_x(int *p_ret, int *f_tab, long long *tmp)
{
	if (f_tab[5] > f_tab[6])
		f_tab[5] = f_tab[5] - 2;
	...
	if (f_tab[6] != 0 || (f_tab[6] == 0 && *tmp > 0))
		ft_putnbr_hex_l(*tmp, p_ret);
	if (f_tab[3])
		while (f_tab[5]-- > ft_countdigits_hex(*tmp))
			*p_ret += ft_putchar(' ');
}

static void		ft_parse_3_x(int *p_ret, int *f_tab, long long *tmp)
{
	if (*tmp == 0 && f_tab[6] == 0)
		f_tab[5]++;
	if (!f_tab[3])
...
{
	ft_parse_conv_x(ap, f_tab, &tmp);
	if ((tmp != 0 || f_tab[6] >= 0) && f_tab[0])
		ft_parse_1_x(p_ret, f_tab, &tmp);
	else
		ft_parse_3_x(p_ret, f_tab, &tmp);
}
